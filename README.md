...
#### Результат: 
На адресе, который должен быть указан в Вашем README, должен быть запущен сервис предсказания цен - 4 балла.  
В папке docs краткое описание разницы в работе development и production серверов - 4 балла.  
